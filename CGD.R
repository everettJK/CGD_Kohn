...
# Manuscript stats.
o <- subset(intSites, timePoint == 'Y1' & cellType == 'PBMC')
round(sum(sapply(split(o, o$patient), function(x){ n_distinct(x$posid) }, simplify = TRUE, USE.NAMES = TRUE))/4)
max(subset(o, nearestFeature == 'MECOM')$relAbund)
max(o$relAbund)
min(sapply(split(o, o$patient), function(x){ round(vegan::estimateR(x$estAbund, index='chao')[2], 0) }, simplify = TRUE, USE.NAMES = TRUE))

# Average number of sites at Y1
o <- subset(intSites, patient %in% c('Patient 2', 'Patient 4', 'Patient 6', 'Patient 7') & timePoint == 'Y1' & cellType == 'PBMC') %>%
  group_by(patient) %>%
...
badActorPlot <- 
     ...
     dplyr::select(patient, GTSP, cellType, timePoint, estAbund, relAbund, nearestFeature) %>%
...
Neutrophils_maps <- lapply(split(d, d$patient), function(x){
  ...
  ggplot(x, aes(timePoint, labeledNearestFeature, fill = relAbund/100)) +
    ...
    scale_fill_gradient(name = 'Relative abundance',
                        ...
                        limits = c(0, 0.27), breaks = c(0.01, 0.15, 0.25), label = scales::percent_format(accuracy = 0.01))+ 
    labs(x = '', y = '') +
...
grid.arrange(gA, gB, gC, gD, gE, gF, gG, legend, ncol=1)
...
save(list = ls(all.names = TRUE), file = 'report.RData', 
     envir = .GlobalEnv, compress = TRUE, compression_level = 9)
